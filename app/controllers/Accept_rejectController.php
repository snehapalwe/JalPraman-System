...
if($modeldata['action']=="REVERT"){
    if(USER_ROLE == 5) {        // Auth3
        $nextsts = 2;           // revert to Auth2
    } elseif(USER_ROLE == 3) {  // Auth2
        $nextsts = 1;           // revert to Auth1
    } elseif(USER_ROLE == 4) {  // Auth1
        $nextsts = -2;           // revert to initial/new
    }
}else{
        $nextsts = -1;           // revert to Auth1
}
...
$db->update("application_form".($modeldata['db_name']=="oc"?"_oc":""),["status"=>$nextsts]);
$ddb = $modeldata['db_name'];
