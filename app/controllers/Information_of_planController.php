<?php 
/**
 * Information_of_plan Page Controller
 * @category  Controller
 */
class Information_of_planController extends SecureController{
	function __construct(){
		parent::__construct();
		$this->tablename = "information_of_plan";
	}
	/**
     * List page records
	...
	$db->where("rec_id",$_GET['rec_id']);
	    ...
	    $db->where("id",$_GET['oc']); 
	    $ocapp=$db->getOne("application_form_oc");
	    $db->where("user_id",$ocapp['user_id']);
	      $x=$db->getOne("application_form")['id']; 
	    $db->where("rec_id",$x);
	...
	# End of before list statement
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				information_of_plan.id LIKE ? OR 
				information_of_plan.rec_id LIKE ? OR 
				information_of_plan.application_no LIKE ? OR 
				information_of_plan.is_property_distrubution_network_available LIKE ? OR 
				information_of_plan.available_esr_name LIKE ? OR 
				information_of_plan.available_details_of_main_supply_line LIKE ? OR 
				information_of_plan.available_diameter LIKE ? OR 
				information_of_plan.available_pressure LIKE ? OR 
				information_of_plan.distance_of_property_from_main_supply_line LIKE ? OR 
				information_of_plan.proposed_esr_name LIKE ? OR 
				information_of_plan.proposed_details_of_main_supply_line LIKE ? OR 
				information_of_plan.proposed_diameter LIKE ? OR 
				information_of_plan.proposed_length LIKE ? OR 
				information_of_plan.additional_sump_and_pump_house_of_society LIKE ? OR 
				information_of_plan.extending_or_laying_new_pipe_lines_of LIKE ? OR 
				information_of_plan.required_distribution_line LIKE ? OR 
				information_of_plan.approximate_cost_of_required_w_s_network_line LIKE ?
			)";
			$search_params = array(
			...
			$this->view->search_template = "information_of_plan/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("information_of_plan.id", ORDER_TYPE);
		}
		if($fieldname){
		...
		$page_title = $this->view->page_title = "Information Of Plan";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$this->render_view("information_of_plan/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("information_of_plan.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Information Of Plan";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		...
		return $this->render_view("information_of_plan/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$this->rules_array = array(
				...
				'available_diameter' => 'required',
				'available_pressure' => 'required',
				...
				'proposed_diameter' => 'required',
				'proposed_length' => 'required',
...
$db->update("application_form",["status"=>2]);
		...
		$page_title = $this->view->page_title = "Add New Information Of Plan";
		$this->render_view("information_of_plan/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("information_of_plan.id", $rec_id);;
				$bool = $db->update($tablename, $modeldata);
				$numRows = $db->getRowCount(); //number of affected rows. 0 = no record field updated
				if($bool && $numRows){
					$this->set_flash_msg("Record updated successfully", "success");
					return $this->redirect("information_of_plan");
				}
				else{
					if($db->getLastError()){
						$this->set_page_error();
		...
		$page_title = $this->view->page_title = "Edit  Information Of Plan";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("information_of_plan/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("information_of_plan.id", $arr_rec_id, "in");
